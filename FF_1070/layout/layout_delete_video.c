...
void  create_delete_video(lv_obj_t *center_cont)
{
    
   	static obj_click_data btn_data = obj_click_data_up_create(delete_video_yes_btn_up);
	static obj_click_data btn_data2 = obj_click_data_up_create(video_delete_no_btn_up);
	message_box_create(center_cont, str_get(SETTING_DELETE_VIDEO_ID), &btn_data, &btn_data2);
...
static void LAYOUT_ENTER_FUNC(delete_video)
{
	...
	create_delete_video(center_cont);
}



static void LAYOUT_QUIT_FUNC(delete_video)
{
	user_data_save();

}

CREATE_LAYOUT(delete_video);
