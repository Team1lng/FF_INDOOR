...
static void delete_flash_picture_yes_btn_up(lv_obj_t *obj)
{
    if (media_file_delete_all_state())
    {
        return;
...
static void flash_photo_delete_no_btn_up(lv_obj_t *obj)
{
	goto_layout(pLAYOUT(sd));
}

void  create_delete_flash_photo(lv_obj_t *center_cont)
{
    
   	static obj_click_data btn_data = obj_click_data_up_create(delete_flash_picture_yes_btn_up);
	static obj_click_data btn_data2 = obj_click_data_up_create(flash_photo_delete_no_btn_up);
	message_box_create(center_cont, str_get(SETTING_DELETE_FLASH_PHOTO_ID), &btn_data, &btn_data2);
...
static void LAYOUT_ENTER_FUNC(delete_flash_photo)
{
	...
	create_delete_flash_photo(center_cont);
}



static void LAYOUT_QUIT_FUNC(delete_flash_photo)
{
	user_data_save();

}

CREATE_LAYOUT(delete_flash_photo);
