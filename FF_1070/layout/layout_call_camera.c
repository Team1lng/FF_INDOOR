...
static void calling_play_start_cb(int index)
{
	...
	/* The intercom preemption path turns PA off before entering this page. */
...
static void calling_back_btn_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_click(back_icon_obj, false); /* 禁用点击，穿透到全屏退出 */
...
lv_obj_t *calling_container_create(lv_obj_t *parent, obj_click_data *yes_btn_data, layout_lang_id id)
{
    ...
    lv_label_set_text(msg_label, str_get(COMMON_LANG_APARTMENT_DOOR_ID)); // 删除提示语
    ...
    // 5. 确认按钮 - 唯一可点击的区域；扩大点击范围至整条按钮区
    lv_obj_t *yes_btn = lv_obj_create(btn_area, NULL);
    lv_obj_set_size(yes_btn, 466, 80);
...
static void LAYOUT_ENTER_FUNC(calling)
{
    ...
    calling_container_create(parent, &yes_btn_data, COMMON_LANG_APARTMENT_DOOR_ID);
    /* 全屏点击即退出（与 yes_btn 行为一致）：父对象即屏幕，绑定同一 yes_btn_data */
    lv_obj_set_click(parent, true);
    obj_click_event_listen(parent, &yes_btn_data);
    int system_vol = user_data_get()->setting.door_ring_volume;
    ring_volume_set(system_vol);
    ringplay_play_form_index(user_data_get()->setting.apartment_door_tone, 100,
...
{
    /* The screen object is reused by every layout. Clear the full-screen
     * callback before another page can receive a blank-area click. */
    obj_click_event_listen(lv_scr_act(), NULL);
    /* Do not block goto_layout() while the doorbell audio worker drains. */
